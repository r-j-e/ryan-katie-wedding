          ...
          {links.map((l, i) =>
            <li key={l.id} style={{ display: 'flex', alignItems: 'center' }}>
              <a href={`#${l.id}`} onClick={go(l.id)} style={{
                ...
                // Over the dark hero video (not yet scrolled) the links are
                // cream; once the cream nav bar fades in, they go to ink.
                color: scrolled
                  ? (active === l.id ? 'var(--burgundy)' : 'var(--ink)')
                  : (active === l.id ? 'var(--blush)' : 'rgba(244,237,228,0.92)'),
                fontSize: 13, fontWeight: 400, letterSpacing: '0.02em',
                fontFamily: "'Cinzel', Georgia, serif",
                fontStyle: active === l.id ? 'italic' : 'normal',
                textShadow: scrolled ? 'none' : '0 1px 12px rgba(16,12,14,0.55)',
                transition: 'color 0.2s',
                whiteSpace: 'nowrap'
              }}>{l.label}</a>
              {i < links.length - 1 &&
                <span style={{ width: 3, height: 3, background: scrolled ? 'var(--ink-mute)' : 'rgba(244,237,228,0.6)', opacity: 0.4, borderRadius: '50%' }} />
              }
            </li>
          )}
        </ul>
...
// ============================================================
// ============================================================
// HERO VIDEO — full-bleed venue footage behind the title card.
// Desktop loads the landscape cut, phones the vertical cut (only the
...
const HeroVideo = () => {
  ...
  return (
    <div aria-hidden="true" style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#14100F' }}>
      <div style={{
      ...
      {/* Scrim — stronger top & bottom (nav, eyebrow, scroll cue), lighter
          through the middle where the names sit. Tuned to the approved still. */}
      <div style={{
        position: 'absolute', inset: 0,
        background: 'linear-gradient(180deg, rgba(16,12,14,0.62) 0%, rgba(16,12,14,0.34) 42%, rgba(16,12,14,0.36) 60%, rgba(16,12,14,0.66) 100%)',
      }} />
    </div>
  );
};
...
const Hero = ({ showCountdown = true, onRsvp }) => {
  ...
  return (
    <section id="top" className="hero-on-video" style={{
      position: 'relative',
      background: '#14100F', padding: '120px 24px 80px',
      ...
      <div style={{
        ...
        {/* Names — Katie & Ryan, hand-written on load (cream over the video) */}
        <h1 style={{
          margin: 0,
          fontSize: 'clamp(28px, 4.4vw, 60px)',
          fontWeight: 300, lineHeight: 1.4, color: 'var(--cream)', letterSpacing: '-0.005em',
          padding: '0 16px',
          filter: 'drop-shadow(0 2px 22px rgba(16,12,14,0.45))',
        }}>
          <NamesWritten />
        </h1>
        ...
        <div className="hero-anim-4" style={{
          ...
          color: 'rgba(244,237,228,0.9)', lineHeight: 1.7,
          textShadow: '0 1px 16px rgba(16,12,14,0.6)',
        ...
        <div className="hero-anim-5 hero-countdown" style={{
          marginTop: 62,
          display: 'inline-flex', alignItems: 'baseline', gap: 18,
          color: 'var(--cream)',
          textShadow: '0 1px 18px rgba(16,12,14,0.5)',
        }}>
        ...
        <div className="hero-anim-6" style={{ marginTop: 48, textAlign: 'center' }}>
          <button
            ...
            style={{
              ...
              boxShadow: '0 6px 28px -8px rgba(16,12,14,0.6)',
              transition: 'background 0.3s ease, letter-spacing 0.3s ease',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.letterSpacing = '0.5em'; e.currentTarget.style.textIndent = '0.5em'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = 'var(--cream)'; e.currentTarget.style.letterSpacing = '0.4em'; e.currentTarget.style.textIndent = '0.4em'; }}
          >RSVP</button>
        </div>
        ...
        <div className="hero-anim-6" style={{ marginTop: 32, textAlign: 'center' }}>
          <button
            ...
            style={{
              ...
              textShadow: '0 1px 14px rgba(16,12,14,0.6)',
            }}>
            <span style={{
              fontFamily: 'Cinzel, Georgia, serif',
              fontSize: 10, fontWeight: 400,
              letterSpacing: '0.36em', textTransform: 'uppercase',
              color: 'rgba(244,237,228,0.85)', opacity: 0.9
            }}>Scroll</span>
      ...
      <style>{`
        ...
        @keyframes driftIn {
          ...
          to   { opacity:0.95; letter-spacing: 0.01em; }
        }
...
<span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
    ...
    fontSize: 'clamp(22px, 3vw, 32px)', fontWeight: 300, lineHeight: 1, color: 'var(--cream)',
    ...
    letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(244,237,228,0.72)',
    textIndent: '0.12em'
  }}>{label}</span>
  </span>;
...
const CdSep = () =>
<span style={{
  width: 1, alignSelf: 'stretch', background: 'var(--cream)', opacity: 0.3
}} />;
...
<div className={className} style={{
  ...
  color: 'var(--cream)',
  textShadow: '0 1px 16px rgba(16,12,14,0.55)'
}}>
